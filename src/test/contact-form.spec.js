...
describe("ContactForm Component", () => {
  let wrapper, contactData, fields, onChange, onSubmit, options;
  ...
  it("should call onChanges with updated contact data when `select` field changes", () => {
    ...
    let keys = options.map((option) => option.id.toFixed());
  ...
  function testField(name, field, key, changedValue) {
    ...
    field.simulate("change", { target: { name: name, value: changedValue } });

    contactData = { ...contactData, [name]: changedValue };
  ...
  it("should render options with labels using component`s `options` values", () => {
    let optionsConfigured = options.map((option) => option.label);
